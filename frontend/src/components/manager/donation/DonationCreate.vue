...
const donationForm = ref({
    ...
    donation_type_id: '', // ✅ correct field
    donation_date: '',
    amount: '',
    recipient_name: '',
    description: '',
...
watch(
    ...
    (newVal) => {
        if (props.editMode && newVal) {
            donationForm.value = {
                category_id: newVal.category_id ?? '', // ✅ correct
                donation_type_id: newVal.donation_type_id ?? '', // ✅ correct
                donation_date: newVal.donation_date ?? '',
...
// ✅ Create or Update Donation (API Call)
...
<template>
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4"
        ...
        <div
            ...
            <!-- Header -->
            <div
                class="sticky top-0 z-10 bg-white flex items-center justify-between px-6 py-4 border-b border-slate-200">
            ...
            <div class="px-6 py-6 space-y-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select v-model="donationForm.category_id" required
                            class="w-full px-4 py-2 rounded-lg border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500">

                            <!-- sirf Donation show hogi -->
                            <option value="">Select Category</option>
                            <option v-for="cat in categorys.filter(c => c.name === 'Donation')" :key="cat.id"
                                :value="cat.id">
                                {{ cat.name }}
                            </option>
                        </select>
                    ...
                    <!-- ✅ Donation Type Dropdown (unchanged except label fix) -->
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">
                            Amount / Value <span class="text-red-500">*</span>
                        </label>
                        <input v-model="donationForm.amount" type="number" placeholder="0.00" min="0" step="0.01"
                            required
                            class="w-full px-4 py-2 rounded-lg border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    ...
                    <div>
                        ...
                        <input v-model="donationForm.recipient_name" type="text" placeholder="Enter recipient name"
                            required
                            class="w-full px-4 py-2 rounded-lg border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                ...
                <div>
                    ...
                    <textarea v-model="donationForm.description" rows="3"
                        placeholder="Write purpose or details about this donation..."
