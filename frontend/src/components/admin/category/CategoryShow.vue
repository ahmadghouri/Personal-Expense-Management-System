...
<template>
    <div>
                ...
                <table class="min-w-full bg-white text-sm text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="category in categorys" :key="category.id"
                            ...
                            <td class="px-6 py-4 text-gray-700">{{ category.description || 'N/A' }}</td>
                            ...
                            <td class="px-6 py-4">
                                <span class="px-3 py-1 rounded-full text-xs font-semibold" :class="{
                                    'bg-green-500 text-white': Number(category.expenses_sum_amount) === 0,
                                    'bg-yellow-500 text-white': Number(category.expenses_sum_amount) > 0,
                                    'bg-gray-400 text-white': category.expenses_sum_amount == null
                                }">
                                    {{ Number(category.expenses_sum_amount) > 0 ?
                                        formatAmount(category.expenses_sum_amount) :
                                        'N/A' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                ...
                                <button class="text-red-500 hover:text-red-700 font-medium"
                                    @click="handleDelete(category.id)">Delete</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <!-- Modal -->
        <CategoryCreate v-if="showModal" :category="selectedUser" @close="closeModal" :handleCategory="props.handleCategory" />
    </div>
</template>
